...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, ScreenHeight, ScreenWidth, 82)];
        _bottomView.backgroundColor = [UIColor clearColor];
        [self addSubview:_bottomView];
        
        UIView *bgView = [[UIView alloc] initWithFrame:CGRectMake(0, 2, ScreenWidth, 80)];
        ...
        record.frame = CGRectMake(ScreenWidth / 2.0 - 30, 12, 60, 60);
...
//        [record addTarget:self action:@selector(cancelRecord) forControlEvents:UIControlEventTouchUpOutside];
        ...
        _recordBgImage = [[UIImageView alloc] initWithFrame:_recordBtn.bounds];
        ...
        [_recordBtn addSubview:_recordBgImage];
        ...
        [_recordBtn addSubview:_recordPromt];
        _recordPromt.center = CGPointMake( _recordBtn.frame.size.width / 2.0, - (_recordPromt.frame.size.height / 2.0));
        ...
        [_recordBtn addSubview:_cancelLabel];
...
//    CGPoint recordBgImagePoint = [self.recordBgImage convertPoint:point fromView:self];
    ...
    _cancelLabel.frame = CGRectMake(point.x + 30, point.y - 60, 80, 30);
    if (isCancel) {
...
- (void)cancelIshidden:(BOOL)isHidden
{
    ...
    _durView.frame = CGRectMake(0, -_durView.frame.size.height, ScreenWidth, _durView.frame.size.height);
    _progressView.progress = 0;
    NSLog(@"取消录制");
    _recordBgImage.backgroundColor = [UIColor colorWithWhite:1.0 alpha:0.9];
    _durLabel.text = @"00:00";
...
- (void)hiddenAnimo:(BOOL)isHidden
{
    ...
    if (isHidden) {
        [UIView animateWithDuration:0.3 animations:^{
            _bottomView.frame = CGRectMake(0, ScreenHeight, ScreenWidth, 82);
        } completion:^(BOOL finished) {
            self.hiddenRecordView(YES);
        }];
    } else {
        [UIView animateWithDuration:0.3 animations:^{
            _bottomView.frame = CGRectMake(0, ScreenHeight - 82, ScreenWidth, 82);
        } completion:^(BOOL finished) {
